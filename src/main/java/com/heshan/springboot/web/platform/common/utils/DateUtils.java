...
public class DateUtils {
    ...
    public static String formatUnixDateMis(Date date) {
        if (null == date) {
            return null;
        }
    ...
    /**
     * 根据参数的日期获取 参数日期23时59分59秒999毫秒的时间
     * @param date 日期字符串
     * @param fmt 日期格式化
     * @return
     */
    public static Long getDayEndTime(String date, String fmt) {
        if (StringUtils.isBlank(date) || StringUtils.isBlank(fmt)) {
            return null;
        }
        SimpleDateFormat sdf = new SimpleDateFormat(fmt);
    ...
    /**
     * 精确计算两个时间差（结束-开始）的天数，不足24小时的不算一天
     * @param msBegin 毫秒-开始
     * @param msEnd 毫秒-结束
     * @return
     */
    public static Integer diffPreciseDays(Long msBegin, Long msEnd) {
        Long diff = (msEnd - msBegin) / (1000 * 3600 * 24);
